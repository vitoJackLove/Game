...
namespace Game
{
    ...
    {
        private const string DataTablePath = "Assets/ABResources/DataTables";
        private const string ConfigDataTablePath = "Assets/ABResources/Configs";
        public const string CSharpCodePath = "Assets/Scripts/Runtime/DataTable";
        private const string CSharpCodeTemplateFileName = "Assets/ABResources/Configs/DataTableCodeTemplate.txt";
        private static readonly Regex EndWithNumberRegex = new Regex(@"\d+$");
        private static readonly Regex NameRegex = new Regex(@"^[A-Z][A-Za-z0-9_]*$");

        public static DataTableProcessor CreateDataTableProcessor(string dataTableName)
        {
            return new DataTableProcessor(GameFramework.Utility.Path.GetCombinePath(DataTablePath, dataTableName + ".txt"), Encoding.Default, 2, 1, null, 3, 4, 1);
        }
        ...
        {
            string binaryDataFileName = GameFramework.Utility.Path.GetCombinePath(ConfigDataTablePath, "DefaultConfig.bytes");

            DataTableProcessor dataTableProcessor = new DataTableProcessor(GameFramework.Utility.Path.GetCombinePath(ConfigDataTablePath, "DefaultConfig.txt"),
                Encoding.Default, 1, 2, null, 3, 4, 1);
        ...
        {
            string binaryDataFileName = GameFramework.Utility.Path.GetCombinePath(DataTablePath, dataTableName + ".bytes");
            if (!dataTableProcessor.GenerateDataFile(binaryDataFileName, Encoding.UTF8) && File.Exists(binaryDataFileName))
            {
                File.Delete(binaryDataFileName);
            }
        ...
        public static void GenerateCodeFile(DataTableProcessor dataTableProcessor, string dataTableName)
        {
            ...
            string csharpCodeFileName = GameFramework.Utility.Path.GetCombinePath(CSharpCodePath, "DR" + dataTableName + ".cs");
        ...
        private static void DataTableCodeGenerator(DataTableProcessor dataTableProcessor, StringBuilder codeContent, object userData)
        {
            ...
            codeContent.Replace("__DATA_TABLE_NAME_SPACE__", Global.NAMESPACE);
        ...
        private static string GenerateDataTableStringParser(DataTableProcessor dataTableProcessor)
        {
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                else if(dataTableProcessor.GetSpecialType(i)  == SpecialType.Enum)
                {
                    string languageKeyword = dataTableProcessor.GetLanguageKeyword(i);
                    stringBuilder.AppendFormat("            {0} = ({1}){2}.Parse(typeof({3}),columnTexts[index++]);", dataTableProcessor.GetName(i),languageKeyword, dataTableProcessor.GetType(i),languageKeyword).AppendLine();
                }
                else if(dataTableProcessor.GetSpecialType(i)  == SpecialType.List)
                {
                    stringBuilder.AppendFormat("            {0} = DataTableExtension.Parse{1}(columnTexts[index++]);", dataTableProcessor.GetName(i),
                        dataTableProcessor.GetMethodName(i)).AppendLine();
                }
                else
                {
                    stringBuilder.AppendFormat("            {0} = DataTableExtension.Parse{1}(columnTexts[index++]);", dataTableProcessor.GetName(i),
        ...
        {
            bool isExistPropertyData = false;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder
            ...
            {
                //数据存在Property数据
                if (dataTableProcessor.GetName(i).StartsWith("Property"))
                {
                    isExistPropertyData = true;
                    continue;
                }
                if (dataTableProcessor.IsCommentColumn(i))
                {
                ...
                if (dataTableProcessor.GetSpecialType(i) == SpecialType.Enum)
                {
                    string languageKeyword = dataTableProcessor.GetLanguageKeyword(i);
                    stringBuilder.AppendFormat("            {0} = ({1}){2}.Parse(typeof({3}),binaryReader.ReadString());", dataTableProcessor.GetName(i),languageKeyword, dataTableProcessor.GetType(i),languageKeyword).AppendLine();
                }
                else if (dataTableProcessor.GetSpecialType(i) == SpecialType.List)
                {
                    stringBuilder.AppendFormat("                    {0} = DataTableExtension.Parse{1}(binaryReader.ReadString());", dataTableProcessor.GetName(i), dataTableProcessor.GetMethodName(i)).AppendLine();
                }
                else
                {
        ...
        private static string GenerateDataTablePropertyArray(DataTableProcessor dataTableProcessor)
        {
            ...
            foreach (PropertyCollection propertyCollection in propertyCollections)
            {
                ...
                stringBuilder
                    ...
                    .AppendLine("            }")
                    .AppendLine()
                    .AppendFormat("            throw new GameFrameworkException(Utility.Text.Format(\"Get{0} with invalid id '{{0}}'.\", id.ToString()));", propertyCollection.Name)
                    .AppendLine()
                    .AppendLine("        }")
                    .AppendLine()
                    ...
                    .AppendLine("            {")
                    .AppendFormat("                throw new GameFrameworkException(Utility.Text.Format(\"Get{0}At with invalid index '{{0}}'.\", index.ToString()));",
            ...
            foreach (PropertyCollection propertyCollection in propertyCollections)
            {
                ...
                for (int i = 0; i < itemCount; i++)
                {
                    ...
                    stringBuilder.AppendFormat("                new KeyValuePair<int, {0}>({1}, {2}),", propertyCollection.LanguageKeyword, item.Key.ToString(), item.Value)
                        .AppendLine();
                }
